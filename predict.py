...
class Predictor(BasePredictor):
    ...
    def predict(
        ...
        # ),
        caption: str = Input(
            description="Desired caption for the images.",
            default=None,
        # ),
        ...
        text = self.text_processors(caption)
        response = self.model({"image": image, "text_input": text}, match_head="itm")
        itm_scores = torch.nn.functional.softmax(response, dim=1)
